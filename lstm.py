...
# Переворачиваем порядок данных, если они упорядочены по убыванию
df = df.iloc[::-1]
# take close price column[4]]
all_y = df['closePrice'].values
...
print(len(testPredict))
print(testPredict)
# Ensure testPrices and testPredict have the same length
testPrices = testPrices[:len(testPredict)]

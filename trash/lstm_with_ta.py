...
look_back = 30                           #начать с 30 и постепенно увеличивать до 240
...
model = Sequential()
model.add(LSTM(200, return_sequences=True, input_shape=(look_back, trainX.shape[2])))  # не 5 а 50 н-р
...
model.fit(trainX, trainY, epochs=5, batch_size=120, verbose=1, callbacks=[early_stopping])  #эпох 100-200
...
# Визуализация
plt.plot(scaler.inverse_transform(dataset)[:, 0][-100:], label='Actual Prices')
#plt.plot(trainPredictPlot, label='Train Predictions')
plt.plot(testPredictPlot[-100:], label='Test Predictions')
plt.legend()
